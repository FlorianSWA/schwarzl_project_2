...
void Sender::operator()(string message) {
    ...
    random_device rd{};
    mt19937 gen{rd()};
    uniform_int_distribution<int> sleep_dis{4, 6};

    while (true) {
        this_thread::sleep_for(chrono::seconds(sleep_dis(gen)));
        ...
        for (int i{0}; i < dv.vector_size; i++) {
            ...
            if (new_target != this->sender_port) {
                send_text(message, new_target, sender_port);
            }
        }
    }
}
...
void Sender::redirect(proto_messages::WrapperMessage message_) {
    ...
    if (strm) {
        ...
        spdlog::info("[{}] Redirect message to {}", "Node " + to_string(sender_port), next_hop);
        strm.close();
        spdlog::debug("Closed connection between Node {} (sender) and Node {} (reciever).", sender_port, next_hop);
    } else {
        spdlog::error("Error occured while connecting: {}", strm.error().message());
...
void Sender::send_update(int target_port_) {
    ...
    if (strm) {
        ...
        spdlog::info("[{}] sent vector update to {}", "Node " + to_string(sender_port), target_port_);
        strm.close();
